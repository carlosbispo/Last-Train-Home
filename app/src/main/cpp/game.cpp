...
Font *speech_font;
float game_scale_x = 1;
float game_scale_y = 1;
...
//  if (app_keypress(KEY_ESCAPE)) app_quit();
  ...
  gfx_push();
  gfx_set_color(1,1,1,1);
  gfx_scale(game_scale_x, game_scale_y);
...
void start() {
    ...
    c = {0};
    c.width = GAME_WIDTH * game_scale_x;
    c.height = GAME_HEIGHT * game_scale_y;
    c.step = frame;
    c.load = load;
    c.resume = resume;
    reset_game();
...
void set_scale(float scale_x, float scale_y) {
  game_scale_x = scale_x;
  game_scale_y = scale_y;
}
...
void on_key_right(bool state) {
    on_key(KEY_RIGHT, state);
}

void on_key_left(bool state) {
  on_key(KEY_LEFT, state);
}

void on_key_select(bool state) {
  on_key(KEY_X, state);
  on_key(KEY_DOWN, state);
}
